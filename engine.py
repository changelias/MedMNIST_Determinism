...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, size, download, model_flag, resize, as_rgb, model_path, trials, determinism, benchmark, seed):
    ...
    determ_var = 'determinism_false'
    benchmark_var = 'benchmark_false'
    
    if determinism:
        determ_var = 'determinism_true'
    if benchmark:
        benchmark_var = 'benchmark_true'
    
    output_root = os.path.join(output_root, determ_var+"_"+benchmark_var, data_flag, time.strftime("%y%m%d_%H%M%S"))
